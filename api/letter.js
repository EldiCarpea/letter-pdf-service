...
const WINDOW_MM = { left: 20, top: 45, width: 90, height: 45 }; // DL/C6/5 links
...
module.exports = async (req, res) => {
  try {
    ...
    const b = req.body || {};
    const adresse = (b.adresse ?? b.address ?? '').toString().trim();
    const plzOrt  = (b['plz/ort'] ?? b.plzOrt ?? b.plz_ort ?? b.plzort ?? '').toString().trim();
...
T: +43 1 774 20 32 · E: info@wisehomes.at · W: wisehomes.at`;
    ...
    // Fensteradresse (editierbar)
    const winX = mm2pt(WINDOW_MM.left), winW = mm2pt(WINDOW_MM.width), winH = mm2pt(WINDOW_MM.height);
    const winY = A4.height - mm2pt(WINDOW_MM.top) - winH;

    const addrField = form.createTextField('anschrift');
    addrField.enableMultiline(); addrField.setFontSize(12); addrField.setBorderWidth(0);
    ...
    // Kontaktzeile (Links)
    let x = marginLeft; const baseY = y;
    const contact = [
    ...
    for (const [label, val, href] of contact) {
      page.drawText(label, { x, y: baseY, size, font: fontRegular }); x += fontRegular.widthOfTextAtSize(label, size);
      page.drawText(val,   { x, y: baseY, size, font: fontRegular, link: href }); x += fontRegular.widthOfTextAtSize(val, size);
    }
